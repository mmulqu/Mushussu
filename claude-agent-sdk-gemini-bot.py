...
# =============================================================================
# Discord Communication Tools (MCP) - Used by both Claude and Gemini
# =============================================================================
...
@tool("restart_self", "Restart the Thoth agent. Use for updates, fresh starts, or switching modes.", {"mode": "Optional[str]: 'claude' or 'gemini'. Restarts in the specified mode."})
async def restart_self_tool(args):
    """
    Request a restart. Can also be used to switch between Claude and Gemini modes.
    ...
    if mode in ("claude", "gemini"):
        # This is the temp directory provided at startup.
        temp_dir = Path("C:\\Users\\Mike\\.gemini\\tmp\\983af8c4b9c573134d637ca39a8d92fc6ccf8eac3f3aa65e9b9fc1289b359540")
        restart_mode_file = temp_dir / "restart_mode.txt"
        try:
            restart_mode_file.parent.mkdir(exist_ok=True)
            restart_mode_file.write_text(mode, encoding="utf-8")
            debug_log(f"Set restart mode to '{mode}' in {restart_mode_file}")
            return {"content": [{"type": "text", "text": f"Restart requested into {mode} mode. Will restart after response completes."}]}
        except Exception as e:
            debug_log(f"Error setting restart mode: {e}")
            return {"content": [{"type": "text", "text": f"Error setting restart mode: {e}"}]}

    return {"content": [{"type": "text", "text": "Restart requested. Will restart after response completes."}]}
...
discord_tools_server = create_sdk_mcp_server(
    name="discord",
    version="1.0.0",
    tools=[send_message_tool, react_tool, send_image_tool, restart_self_tool, apply_update_tool],
)
...
- restart_self(mode="<claude|gemini>") - Restart yourself, optionally switching modes.
    ...
    try:
        ...
        for call_str in tool_calls:
            ...
            if tool_name == "send_message":
                await send_message_tool(args)
            elif tool_name == "react":
                await react_tool(args)
            elif tool_name == "send_image":
                await send_image_tool(args)
            elif tool_name == "restart_self":
                await restart_self_tool(args)
            elif tool_name == "apply_update":
                await apply_update_tool(args)
            else:
